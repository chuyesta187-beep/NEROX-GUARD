// =====================================================================
// 🛡️ NEROX GUARD — DEPLOYMENT READY CORE v5.6 (RENDER PRODUCTION)
// =====================================================================
const { 
    Client, 
    GatewayIntentBits, 
...
const client = new Client({
    intents: [
        ...
        GatewayIntentBits.AutoModConfiguration,
        GatewayIntentBits.AutoModExecution,
        GatewayIntentBits.GuildVoiceStates
    ],
    partials: [Partials.Message, Partials.Channel, Partials.GuildMember]
});

// Extracción estricta de entorno para Render
...
// Emisor de logs premium estilo AMOLED/Black seguro
...
function generarPanelEmbed(config, guildName) {
    ...
    return new EmbedBuilder()
        ...
        .setTitle('🛡️ CENTRAL DE CONTROL MAESTRA — NEROX GUARD v5.6')
        .setDescription(`Panel de administración global para la protección activa de **${guildName}**.\n\n⚠️ *Nota de Entorno: Los respaldos locales en Render son volátiles. Se recomienda conectar un volumen persistente o base de datos externa para producción.*`)
        .addFields(
...
client.once('ready', async () => {
    console.log(`🛡️ Nerox Guard v5.6 iniciado sin conflictos de Intents para: ${client.user?.tag}`);
    
    // El despliegue de comandos globales solo se ejecuta si se requiere de forma manual para evitar rate limits en reinicios frecuentes
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isStringSelectMenu()) {
        ...
        if (seleccion === 'setup_backup_load') {
            ...
            // Canales de texto/voz secundarios
            for (const ch of datosCanales.filter(c => c.type !== ChannelType.GuildCategory)) {
                let existe = guild.channels.cache.find(e => e.name === ch.name && e.type === ch.type);
                if (!existe && guild.channels.cache.size < 500) {
                    try {
                        ...
                        // Propiedades sanitizadas según tipo de canal para evitar fallos de API
                        const opcionesCanal = {
                            name: ch.name,
                            type: ch.type,
...
client.on('messageCreate', async message => {
    ...
    // Interceptor de anuncios del Owner
...
// Logs: Ingreso de Miembros (Anti-Raid / Anti-Bot integrado)
...
async function verificarSospechoso(guild, executorId, actionType) {
    ...
    // Validación estricta de jerarquía de roles para evitar crasheos por falta de permisos relativos
    ...
    if (atacante && me.roles.highest.position <= atacante.roles.highest.position) {
        await enviarLog(guild, 0xFF0000, '⚠️ BRECHA ANTI-NUKE DETECTADA', `El ejecutor <@${executorId}> está cometiendo acciones sospechosas de **${actionType}**, pero su rol es superior o igual al mío. ¡Imposible ejecutar baneo de mitigación!`);
        return false;
    }
