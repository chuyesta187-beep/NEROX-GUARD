...
// 🧪 CAPTURA CENTRAL DE EXCEPCIONES
...
const commands = [
    ...
    // ⚔️ NUEVAS INCORPORACIONES TÁCTICAS SOLICITADAS
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isChatInputCommand()) {
        ...
        // --- CONTINUACIÓN DEL REPERTORIO CLÁSICO ---
        ...
        if (commandName === 'appeal') {
            if (!isUserBotBanned(user.id)) return interaction.editReply({ content: '❌ No requerido.' });
            ...
            return interaction.editReply({ content: '✅ Apelación enviada.' });
        }
    ...
    if (interaction.isButton()) {
        ...
        if (interaction.customId === 'trigger_captcha_process') {
            ...
            const rRow = new ActionRowBuilder().addComponents([n1+n2, n1+n2+2, n1+n2-1, n1+n2+3].sort(() => Math.random() - 0.5).map(op => new ButtonBuilder().setCustomId(`ans_captcha_${op}`).setLabel(`${op}`).setStyle(ButtonStyle.Secondary)));
...
client.on('messageCreate', async message => {
    ...
    // Filtros de Menciones y Spam Clásicos
...
// INTERCEPTORES DE CONSERVACIÓN ESTRUCTURAL (ANTINUKE DE CANALES Y ROLES)
