...
app.use(session({
    secret: process.env.SESSION_SECRET || "secreto_seguro_nerox_guard_9876",
    resave: false,
...
// 🛠TT PANEL INTERACTIVO DE CONFIGURACIÓN POR SERVIDOR
...
client.once("clientReady", async () => {
    // 🔍 Corregido con backticks (` `) para evitar errores de sintaxis al evaluar variables internas
...
client.on("interactionCreate", async interaction => {
    ...
    try {
        if (interaction.commandName === "ping") {
            await interaction.reply({ content: `🏓 **Pong!** | API Latencia: \`${client.ws.ping}ms\``, ephemeral: true });
        }

        if (interaction.commandName === "kick") {
            if (!interaction.member.permissions.has("KickMembers")) return interaction.reply({ content: "❌ Permisos insuficientes.", ephemeral: true });
            const targetUser = interaction.options.getMember("usuario");
            const reason = interaction.options.getString("razon") || "No especificada.";
            if (!targetUser || !targetUser.kickable) return interaction.reply({ content: "❌ No se puede expulsar a este usuario.", ephemeral: true });
            
            await targetUser.kick(reason);
            await interaction.reply({ content: `✅ **${targetUser.user.tag}** expulsado. Razón: ${reason}` });
        }

        if (interaction.commandName === "ban") {
            if (!interaction.member.permissions.has("BanMembers")) return interaction.reply({ content: "❌ Permisos insuficientes.", ephemeral: true });
            const targetUser = interaction.options.getMember("usuario");
            const reason = interaction.options.getString("razon") || "No especificada.";
            if (!targetUser || !targetUser.bannable) return interaction.reply({ content: "❌ No se puede banear a este usuario.", ephemeral: true });
            
            await targetUser.ban({ reason });
            await interaction.reply({ content: `⛔ **${targetUser.user.tag}** baneado permanentemente. Razón: ${reason}` });
        }

        if (interaction.commandName === "clear") {
            if (!interaction.member.permissions.has("ManageMessages")) return interaction.reply({ content: "❌ Permisos insuficientes.", ephemeral: true });
            const amount = interaction.options.getInteger("cantidad");
            if (amount < 1 || amount > 100) return interaction.reply({ content: "❌ Rango válido: 1 a 100.", ephemeral: true });
            
            await interaction.channel.bulkDelete(amount, true);
            await interaction.reply({ content: `🧹 Mensajes eliminados: \`${amount}\`.`, ephemeral: true });
        }
    } catch (err) {
        console.error(err);
    }
