...
let dbMigrada = false;
if (!db.configs) { db.configs = {}; dbMigrada = true; }
if (!db.whitelists) { db.whitelists = {}; dbMigrada = true; }
if (!db.warns) { db.warns = {}; dbMigrada = true; }
if (!db.globalBlacklist) { db.globalBlacklist = []; dbMigrada = true; }
if (!db.reports) { db.reports = {}; dbMigrada = true; }
if (!db.appeals) { db.appeals = []; dbMigrada = true; }
...
async function verificarNuke(guild, executorId, actionType) {
    ...
    // Corregido: Umbral calibrado a >= 5 acciones para mitigar falsos positivos técnicos
    if (recent.length >= 5) { 
...
// Corregido: Reajustado al evento nativo estable 'ready' compatible con v14 estándar
...
client.on('interactionCreate', async interaction => {
    try {
        ...
        const options = isCommand ? interaction.options : null;
        ...
        if (interaction.isChatInputCommand()) {
            ...
            if (commandName === 'serverbackup') {
                ...
                if (sub === 'create') {
                    ...
                    // Corregido: Guardado explícito de los tipos de canales
                    const estructura = {
                        ...
                        canales: guild.channels.cache.map(c => ({ name: c.name, type: c.type }))
                    };
                ...
                if (sub === 'load') {
                    ...
                    // Corregido: Conversión precisa a BigInt para reconstrucción de bitfield sin corrupción
                    ...
                    // Corregido: Despliegue seguro respetando la tipificación nativa de discord.js v14
            ...
            if (commandName === 'purge') {
                ...
                // Corregido: Bloque catch defensivo para atrapar errores de logs viejos de 14 días
                await channel.bulkDelete(qty, true)
                    .then(deleted => interaction.editReply({ content: `🗑️ Mensajes purgados: **${deleted.size}**.` }))
                    .catch(() => interaction.editReply({ content: '⚠️ Algunos mensajes tienen más de 14 días y no se pueden purgar mediante bulkDelete.' }));
                return;
            }
            if (commandName === 'lock') {
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId.startsWith('ans_captcha_')) {
                ...
                if (res === activeCaptchas.get(user.id)) {
                    // Corregido: Limpieza inmediata del token de sesión en memoria dinámica
        ...
        console.error("🚨 [Error crítico detectado en InteractionCreate]:", err.stack);
        try {
...
// Corregido: Enlace dinámico al puerto asignado por Render (process.env.PORT) para hosting perimetral estable
