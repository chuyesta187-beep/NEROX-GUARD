...
// En producción se conecta a PostgreSQL mediante Prisma
const dbConfig = new Map();
const dbPanels = new Map();      // Paneles ilimitados de tickets
const dbForms = new Map();       // Formularios dinámicos
const dbEconomy = new Map();     // Economía ($)
const dbCustomCmds = new Map();  // Comandos personalizados
const dbStats = new Map();       // Métricas globales y por servidor
...
function getGuildEconomy(guildId, userId) {
    ...
    if (!dbEconomy.has(key)) {
        dbEconomy.set(key, { cash: 100, bank: 0, lastWork: 0, lastDaily: 0 });
    }
...
// ==========================================
// 2. ENTERPRISE EXPRESS DASHBOARD ENGINE
// ==========================================
const app = express();
...
app.use(session({
    secret: process.env.SESSION_SECRET || 'nerox_enterprise_v2_secret',
    resave: false,
...
// Estilos globales Enterprise V2
...
<head>
    ...
    <title>${title} - Nerox Guard Enterprise</title>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" rel="stylesheet">
    <style>
        :root { --bg-main: #0b0f19; --bg-card: #151c2c; --bg-hover: #1e293b; --accent: #38bdf8; --accent-hover: #0284c7; --text: #f8fafc; --text-sub: #94a3b8; --border: #334155; --success: #22c55e; --danger: #ef4444; }
        * { box-sizing: border-box; font-family: 'Segoe UI', system-ui, sans-serif; margin: 0; padding: 0; }
        ...
        .form-group input, .form-group select, .form-group textarea { width: 100%; padding: 10px; background: var(--bg-main); border: 1px solid var(--border); border-radius: 6px; color: #fff; }
    </style>
</head>
<body>
    <div class="sidebar">
        <div>
            <div class="brand"><i class="fa-solid fa-shield-halved"></i> NEROX v2</div>
            <ul class="nav-list">
                <li class="nav-item active"><a href="/dashboard"><i class="fa-solid fa-house"></i> Servidores</a></li>
                <li class="nav-item"><a href="https://discord.gg/PZw45tHPfc" target="_blank"><i class="fa-solid fa-headset"></i> Soporte</a></li>
            </ul>
        </div>
        ${user ? `
...
app.get('/', (req, res) => {
    res.send(`
        ...
        <div style="text-align:center; padding: 100px 20px;">
            <h1 style="font-size:3em; margin-bottom:10px; color:var(--accent);">🛡️ Nerox Guard Enterprise v2</h1>
            <p style="color:var(--text-sub); margin-bottom:30px; font-size:1.2em;">La suite defensiva, económica e interactiva definitiva para comunidades de Discord.</p>
            ...
            <a href="https://discord.gg/PZw45tHPfc" target="_blank" class="btn" style="background:#475569; padding: 15px 30px; font-size: 1.1em; margin-left:10px;"><i class="fa-brands fa-discord"></i> Soporte Oficial</a>
        </div>
        </body></html>
    `);
});

// Selector de Servidores y Métricas Generales
app.get('/dashboard', isAuth, (req, res) => {
    ...
    res.send(`
        ${renderHeader(req.session.user, "Selección de Servidor")}
        <div class="top-bar">
            <h2>Panel de Control Central</h2>
        </div>
        <div class="stats-grid">
            <div class="stat-card"><div>Estado Global</div><div class="val" style="color:var(--success);">🟢 En Línea</div></div>
            <div class="stat-card"><div>Ping WebSocket</div><div class="val">${client.ws.ping}ms</div></div>
            <div class="stat-card"><div>Servidores Protegidos</div><div class="val">${client.guilds.cache.size}</div></div>
            <div class="stat-card"><div>Ataques Bloqueados</div><div class="val">${dbStats.get('attacks') || 0}</div></div>
        </div>
        <h3>Tus Servidores Administrados</h3><br>
        ${listHtml || '<p>No administras ningún servidor actualmente.</p>'}
        </body></html>
    `);
});

// Panel Unificado v2 de un Servidor
app.get('/dashboard/guild/:guildId', isAuth, (req, res) => {
    ...
    res.send(`
        ...
        <div class="top-bar">
            <h2>Configuración Enterprise: ${guild.name}</h2>
            <a href="/dashboard" class="btn" style="background:#475569;">← Volver</a>
        </div>

        <!-- 1. MODULOS DE SEGURIDAD -->
        <div class="card">
            <h3><i class="fa-solid fa-shield-halved" style="color:var(--accent);"></i> 🛡️ Módulos de Seguridad Superior</h3><br>
            <form action="/dashboard/guild/${guildId}/security" method="POST" class="grid-2">
                ...
                <div style="grid-column: 1 / -1; margin-top:10px;">
                    <button class="btn" type="submit"><i class="fa-solid fa-floppy-disk"></i> Guardar Seguridad</button>
                </div>
            </form>
        </div>

        <!-- 2. SISTEMA DE TICKETS ILIMITADOS -->
        <div class="card">
            <h3><i class="fa-solid fa-ticket" style="color:var(--accent);"></i> 🎫 Paneles de Soporte Ilimitados</h3><br>
            <div style="margin-bottom:20px;">
                ${panels.map((p, idx) => `
                    <div style="display:flex; justify-content:space-between; align-items:center; background:var(--bg-main); padding:12px; border-radius:8px; margin-bottom:8px;">
                        <div>
                            <strong>${p.title}</strong> - <span style="color:var(--text-sub);">Canal: <#${p.channelId}></span>
                        </div>
                        <form action="/dashboard/guild/${guildId}/tickets/deploy" method="POST" style="display:inline;">
                            ...
                            <button class="btn" type="submit" style="padding:6px 12px; font-size:0.85em;"><i class="fa-solid fa-paper-plane"></i> Enviar a Discord</button>
                        </form>
                    </div>
            ...
            <form action="/dashboard/guild/${guildId}/tickets/create" method="POST" class="form-group">
                <div class="grid-2">
                    <div>
                        ...
                        <input type="text" name="title" placeholder="Ej: CENTRO DE SOPORTE" required>
                    </div>
                    <div>
                        <label>ID Canal de Envío</label>
                        <input type="text" name="channelId" placeholder="ID del canal de texto" required>
                    </div>
                </div>
                <div style="margin-top:10px;">
                    <label>Descripción del Embed</label>
                    <textarea name="description" placeholder="Instrucciones para abrir tickets..."></textarea>
                </div>
        ...
        <div class="card">
            <h3><i class="fa-solid fa-coins" style="color:var(--accent);"></i> 💰 Sistema Económico Personalizado</h3><br>
            <form action="/dashboard/guild/${guildId}/economy" method="POST" class="grid-2">
                <div class="form-group">
                    <label>Nombre de Moneda</label>
                    <input type="text" name="economyName" value="${config.economyName}">
                </div>
                <div class="form-group">
                    <label>Símbolo Moneda</label>
                    <input type="text" name="economySymbol" value="${config.economySymbol}">
                </div>
                <div style="grid-column: 1 / -1;">
                    <button class="btn" type="submit"><i class="fa-solid fa-floppy-disk"></i> Guardar Ajustes Económicos</button>
                </div>
            </form>
        </div>
        </body></html>
    ...
    if (guild && panel) {
        ...
        if (channel && channel.isTextBased()) {
            const embed = new EmbedBuilder()
                ...
                .setDescription(panel.description || 'Haz clic en el botón inferior para abrir un ticket de asistencia.')
                .setColor(0x38BDF8);
...
app.post('/dashboard/guild/:guildId/economy', isAuth, (req, res) => {
    const { guildId } = req.params;
    const { economyName, economySymbol } = req.body;
    const config = getGuildConfig(guildId);
    config.economyName = economyName;
    config.economySymbol = economySymbol;
    dbConfig.set(guildId, config);
    res.redirect(`/dashboard/guild/${guildId}`);
});

app.listen(PORT, () => {
    console.log(`[Dashboard Enterprise v2] Servidor activo escuchando en el puerto ${PORT}`);
});

// ==========================================
// 3. EVENTOS DE DISCORD & INTERACCIONES
// ==========================================
async function handleAntiNukeAction(guild, executor, actionName) {
    ...
    try {
        ...
        if (member) {
            ...
            await member.ban({ reason: `[Nerox Enterprise v2] ${actionName}` }).catch(() => {});
        }
    } catch (err) {
...
// Manejo de Interacciones (Modals, Botones y Slash Commands)
client.on('interactionCreate', async (interaction) => {
    // 1. Abrir Modal de Ticket al Presionar Botón
    if (interaction.isButton() && interaction.customId.startsWith('ticket_open_')) {
        ...
        const inputReason = new TextInputBuilder()
            ...
            .setLabel('¿Cuál es el motivo de tu solicitud?')
    ...
    // 3. Comandos Slash (Economía y Utilidad)
    if (interaction.isChatInputCommand()) {
        ...
        if (commandName === 'work') {
            ...
            if (now - eco.lastWork < 3600000) {
                return interaction.reply({ content: '⏳ Debes esperar 1 hora entre cada turno de trabajo.', ephemeral: true });
            }
            const reward = Math.floor(Math.random() * 400) + 100;
            eco.cash += reward;
            eco.lastWork = now;
            return interaction.reply(`👷 Trabajaste duro y ganaste **${config.economySymbol}${reward}** ${config.economyName}.`);
        }
    }
});
...
client.once('ready', async () => {
    console.log(`[Nerox Guard Enterprise v2] Bot iniciado correctamente como ${client.user.tag}`);
    const commands = [
        { name: 'balance', description: 'Muestra tu saldo actual de dinero ($)' },
        { name: 'work', description: 'Trabaja para obtener dinero ($)' }
    ];
